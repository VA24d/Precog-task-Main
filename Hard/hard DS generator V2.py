...
def distort_image(image, target_width, target_height):
    """
    Apply a random rotation to the image, then crop and resize so that the
    text stays fully within a canvas of (target_width, target_height).
    """
    # Apply a random rotation with expand=1 so that no parts are clipped initially.
    angle = random.uniform(-10, 10)  # Random rotation angle between -10 and 10 degrees
    rotated = image.rotate(angle, expand=1, fillcolor=(255, 255, 255))
    
    # Get the bounding box of the non-transparent (non-empty) region.
    bbox = rotated.getbbox()
    if bbox:
        rotated = rotated.crop(bbox)
    
    # If the rotated image is larger than target, scale it down.
    r_width, r_height = rotated.size
    scale_ratio = min(target_width / r_width, target_height / r_height, 1.0)
    if scale_ratio < 1.0:
        new_size = (int(r_width * scale_ratio), int(r_height * scale_ratio))
        rotated = rotated.resize(new_size, resample_filter)
        r_width, r_height = rotated.size

    # Create a new transparent image of the target size.
    new_img = Image.new("RGBA", (target_width, target_height), (255, 255, 255, 0))
    # Center the rotated image on this canvas.
    offset = ((target_width - r_width) // 2, (target_height - r_height) // 2)
    new_img.paste(rotated, offset, rotated)
...
while generated_count < num_samples and attempts < max_attempts:
    ...
    # Apply distortion if enabled, ensuring the text stays inside the target dimensions.
    if enable_distortion:
        text_img = distort_image(text_img, image_width, image_height)
