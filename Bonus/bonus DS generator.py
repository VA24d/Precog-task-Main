...
# Use a compatible resampling filter (for Pillow 10+ and older versions)
try:
...
def generate_background(bg_type, width, height, font_color=None):
    """
    Generate a background image with a red/green base and noise dots.
      - bg_type: "clean", "black_dots", or "font_dots"
      - font_color: used for dots if bg_type == "font_dots"
      
    Returns:
      (background_image, base_color)
    """
    # Choose a random base color: red or green.
    base_color = random.choice([(255, 0, 0), (0, 255, 0)])
    bg = Image.new("RGB", (width, height), color=base_color)
    draw = ImageDraw.Draw(bg)
    
    if bg_type == "clean":
        return bg, base_color

    # Noise parameters: smaller dots and more numerous
    dot_size = random.choice([1, 2])
    num_dots = random.randint(100, 200)
    
    # Set the dot color: black for "black_dots"; if "font_dots", use the provided font_color.
    dot_color = (0, 0, 0) if bg_type == "black_dots" else (font_color if font_color else (0, 0, 0))

    # Add noise dots over the entire background.
...
while generated_count < num_samples and attempts < max_attempts:
    ...
    # Generate the background image and get its base color.
    background, base_color = generate_background(bg_type, image_width, image_height, font_color)
    
    # For the Bonus Set: if the background is red (i.e. base_color is (255, 0, 0)),
    # then render the word in reverse on the image.
    displayed_word = word if base_color == (0, 255, 0) else word[::-1]
    
    # Generate the text image (with transparent background) ensuring it fits.
    text_img = generate_text_image(displayed_word, chosen_font_path, font_color, image_width, image_height, initial_font_size)
    ...
    # Save the final image with a filename that includes a unique count and the original word.
